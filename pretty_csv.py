...
def main():
    pretty_file("shares.csv", header=False, border=False, delimiter="|")
...
def pretty_file(filename, **options):
    ...
    with open(filename, "r",newline='') as input: #parse the file and determine the width of each column
    ...
    if max_columns > 1:
        ...
        with open(filename, "r",newline='') as input:
            reader=csv.reader(input, **options)
